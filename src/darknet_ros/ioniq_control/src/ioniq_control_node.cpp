...
// TSR
bool init_stop_search_flag = 0;  // when stop signal at first.
double init_SP_distance = 0;     // when init_SP_flag set to 1 distance of Stop Point is written
double init_target_speed = 0;    // when init_SP_flag set to 1, Target Speed is written
double current_SP_distance = 0;
bool SP_passing_flag = 0;
...
//#endif
    ...
    // ROS thread
    pthread_create(&p_thread[5], NULL, ROS_thread, (void *)arg);

    //ros::init(argc,argv,"ioniq_control_node");
    // Decision Code thread
    pthread_create(&p_thread[6], NULL, Decition_thread, NULL);

    pthread_create(&p_thread[7], NULL, Print_Write_thread, NULL);

    //ARG *arg_m = (ARG *)param;
//    ros::init(argc,argv,"ioniq_control_node");
//    ros::NodeHandle nh;
//    ros::Subscriber sub = nh.subscribe("/darknet_ros/bounding_boxes", 1, TSR_MsgCallback);
