...
//    result_num = 0;
    ...
    ARG *arg_m = (ARG *)param;
    ros::init(arg_m->argc,arg_m->argv,"ioniq_control_node");
    ros::NodeHandle nh;
...
void TSR_MsgCallback(const darknet_ros_msgs::BoundingBoxes::ConstPtr &msg){
    ...
    if(nboxes > 0)
    {
       // std::cout << "---------------------------------------" << std::endl;
        ...
        if(("TR_3_R" == msg->bounding_boxes[i].Class) || ("TR_4_R" == msg->bounding_boxes[i].Class)) tsr[i].data_class = 1;
        if(("TR_3_Y" == msg->bounding_boxes[i].Class) || ("TR_4_Y" == msg->bounding_boxes[i].Class)) tsr[i].data_class = 2;
        if(("TR_3_G" == msg->bounding_boxes[i].Class) || ("TR_4_G" == msg->bounding_boxes[i].Class)) tsr[i].data_class = 3;
        if("TR_4_LG" == msg->bounding_boxes[i].Class) tsr[i].data_class = 4;
